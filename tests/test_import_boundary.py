...
from table_transformer_structure_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
...
_CONFIG = json.dumps(
    {"model_type": "table-transformer", "use_timm_backbone": False, "num_queries": 125}
).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        TableTransformerStructurePipeline.from_pretrained(
