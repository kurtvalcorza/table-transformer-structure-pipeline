...
- the profile-specific public-API calls (`stage_missing_files`, `verify_snapshot`,
  ...
  exports, the learner-facing statements (zero-shot, frozen and unfrozen policy, lowest validation loss, grid
...
5. verify every default-path stage completes:
   ...
   - Section 7: `pipe.adapt` printing epoch 0 (the untouched rows) and epoch 1 (the heads) on validation, then 3
...
|---|---|---|---|---|
| `table_transformer_structure_colab.ipynb` (`E2E`) | `__LOCAL_STAMP__` / `__LOCAL_BLOB__` | 2026-09-19 | Local pre-flight harness (Windows, CPython 3.12.10, CPU, `google.colab` shim, pins pre-installed) | PASS — pre-flight only, **not** promotion evidence |
...
the 94 embedded tables decoded and digest-checked, validation and paper-level split, the inference contract with a
per-label `sample-sanity` report on a real table, the grid prior, the zero-shot checkpoint, the three-policy ladder
with validation selection, held-out evaluation, before/after structure, adapter export and reload parity — which
catches defects but is **not** a supported runtime under REL1/REL10, and it ran with the snapshot pre-staged, so the
